...
let refresher = async function(uin, info) {
  ...
  if(existed) return;
  console.debug(`uin:${uin} refresher started`);
  try{
    while(true) {
      ...
      if(Date.now() - uinLastUpdate.get(uin) < 60*1000) {
        console.debug(`uin:${uin} refreshing skipped`);
        continue;
      }
      console.debug(`uin:${uin} refreshing`);
      ...
      if(!responseBody) {
        console.error(`An error occurred during refresh for uin:${uin}, terminate refreshing for this user. `);
        break;
      }
      let result = responseBody.match(/window\.synccheck=\{retcode:\"(\d+)\",selector:\"(\d+)\"\}/);
      if(!result) {
        console.error(`Wrong format returned during refresh for uin:${uin}, terminate refreshing for this user. body is as follows: \n${responseBody}`);
        break;
      }
      if(result[1] !== "0") {
        console.log(`An non-zero retcode(${result[1]}) received during refresh for uin:${uin}, terminate refreshing for this user. `);
        break;
      }
      console.debug(`uin:${uin} refreshed with selector: ${result[2]}`);
    }
  } catch(e) {
    console.error(e);
  } finally {
    console.debug(`uin:${uin} refresher ended`);
...
if(process.env.USE_HTTPS) {
  if(!process.env.SERVER_NAME) {
    console.error('SERVER_NAME need to be specified to use USE_HTTPS option');
    process.exit(1);
  }
  ...
  app.listen(port, () => console.log(`wewechat refresher listening on port ${port}!`));
}
